...
TEST_F(Moisture_2D_TwoElements_2, TestExample_1)
{
    ...
    const auto initialHumidity = 0.99;
    const auto initialPressure = 101325.0;

    State state(initialTemperature, initialHumidity, initialPressure, 0);

    NodePool::Instance().createNode(1, 0.15, 0.05, State(initialTemperature, 0.99996801626559184, initialPressure, 0));
    NodePool::Instance().createNode(2, 0.15, 0, State(initialTemperature, 0.99996801626559184, initialPressure, 0));
    NodePool::Instance().createNode(3, 0.05, 0.05, State(initialTemperature, 0.99994580534423594, initialPressure, 0));
    NodePool::Instance().createNode(4, 0.05, 0, State(initialTemperature, 0.99994580534423594, initialPressure, 0));
    NodePool::Instance().createNode(5, 0, 0.05, State(initialTemperature, 0.99990647350710449, initialPressure, 0));
    NodePool::Instance().createNode(6, 0, 0, State(initialTemperature, 0.99990647350710438, initialPressure, 0));
