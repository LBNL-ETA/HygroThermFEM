...
//! estimator in ErrorEstimator.hxx. That estimator never references the domain; this adapter is
//! the one place that reads a solved domain's elements and assembles the estimator Input.
namespace HygroThermFEM::errorest
{
    ...
    //!
    //! Assembles Input from the domain's elements: the global Gauss-point coordinates, the FE
    //! flux (sigma = -k * grad) at those points, and the per-element inverse conductivity that
    //! defines the energy norm. Elements are grouped for recovery by material so patches do not
    //! smooth flux across a material interface.
    //!
