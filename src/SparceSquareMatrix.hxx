...
namespace FenestrationCommon
{
    ...
    //////////////////////////////////////////////////////////////////////////////
    template<class T>
    class SparceSquareMatrix
    {
    public:
        explicit SparceSquareMatrix(const std::size_t size) : m_size(size)
        {}

        explicit SparceSquareMatrix(const std::initializer_list<std::vector<T>> & t_input) :
            m_size(t_input.size())
        {
            auto i = 0u;
        ...
        /// Makes equivalent upper triangular matrix. It also changes vector so that it can be used
        /// in solver
        void makeUpperTriangular(std::vector<T> & rightSide)
        {
            ...
            for(auto i = 0u; i < m_size - 1; ++i)
            {
                ...
                {
                    if(m_Matrix.exists(j, i) != 0)
                    // if(equation >= i + 1 )
                    {
                        const auto coeff = -m_Matrix.at(i, i) / m_Matrix.at(j, i);
                        ...
                        std::set_union(RowIndex[i].begin(),
                                       RowIndex[i].end(),
                                       RowIndex[j].begin(),
                                       RowIndex[j].end(),
                        ...
                        for(auto & row : unionRange)
                        {
                            ...
                            m_Matrix(j, row) = coeff * m_Matrix.at(j, row) + m_Matrix.at(i, row);
                        }
                    }
                }
            }
        ...
        SparceSquareMatrix<T> mmultRows(const std::vector<T> & t_vector) const
        {
            ...
            for(auto i = 0u; i < t_vector.size(); ++i)
            {
                ...
                {
                    result(i, j) = m_Matrix.at(i, j) * t_vector[i];
                }
            }
            return result;
        }
        ...
        SparceSquareMatrix<T> & operator+(const SparceSquareMatrix<T> & rhs)
        {
            ...
            for(unsigned i = 0; i < m_size; ++i)
            {
                ...
                {
                    m_Matrix(i, j) = m_Matrix.at(i, j) + rhs.m_Matrix.at(i, j);
                }
            }
        ...
        SparceSquareMatrix<T> operator*(const SparceSquareMatrix<T> & rhs)
        {
            ...
            for(auto i = 0u; i < m_size; ++i)
            {
                for(auto k = 0u; k < m_size; ++k)
                {
                    for(auto j = 0u; j < m_size; ++j)
                    {
                        temp(i, j) = temp(i, j) + m_Matrix.at(i, k) * rhs.m_Matrix.at(k, j);
                    }
                }
            }
                                               ...
                                               const double t_Value)
        {
            ...
            for(auto i = 0u; i < lhs.size(); ++i)
            {
                ...
                {
                    aMatrix(i, k) += t_Value * lhs.m_Matrix.at(i, k);
                }
            }
        ...
        friend std::vector<T>
          operator*(const std::vector<T> & t_vector,
                    const SparceSquareMatrix & t_matrix)
        {
            ...
            for(auto i = 0u; i < t_vector.size(); ++i)
            {
                for(auto j = 0u; j < t_vector.size(); ++j)
                {
                    result[i] += t_matrix.m_Matrix.at(j, i) * t_vector[j];
                }
            }
        ...
        friend std::vector<T>
          operator*(const SparceSquareMatrix & t_matrix,
                    const std::vector<T> & t_vector)
        {
            ...
            for(auto i = 0u; i < t_vector.size(); ++i)
            {
                for(auto j = 0u; j < t_vector.size(); ++j)
                {
                    result[i] += t_matrix.m_Matrix.at(i, j) * t_vector[j];
                }
            }
