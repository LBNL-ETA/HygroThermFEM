...
namespace MoisThermFEM
{
    ...
    public:
        ...
        IBCLinear2D() = delete;
        IBCLinear2D(const size_t index1, const size_t index2, const bool t_Linear = true);

        std::vector<std::size_t> getNodeIndexes() const;
        Node2D & getNode(const std::size_t index);

        /// Every boundary condition will return matrices and vectors. There are two type of
        /// matrices. First one is called H matrix and will include sum of all matrices that do not
        /// have first derivative of coefficients. Second one is delta H matrix that is based on
        /// coefficients derivative. Reason for keeping them separate is that matrix equation for
        /// non-linear case need to know what matrices are coming from coefficients derivative,
        /// because they will be on left hand side, next to unknown.
