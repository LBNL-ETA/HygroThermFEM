...
TemperatureBC::TemperatureBC(Node2D &t_Node1, Node2D &t_Node2,
                             const double t_Temp1, const double t_Temp2)
...
BlackBodyRadiationBC::BlackBodyRadiationBC(const Node2D &t_Node1,
    ...
    : IBCLinear2D(t_Node1, t_Node2, false),
      m_RadiationTemperature{t_RadiationTemperature}, m_Emissivity{
                                                          t_Emissivity} {}
...
std::vector<double> MoistureBC::R_Vector() const {
  ...
  auto saturation = SaturationFunction(Property::temperature);
  // pValue airFill = MaterialProperties::getAirFill( m_Material );
  auto humidityCalculator = saturation * m_Material.porosity() * m_AirHumidity;
