...
namespace MoisThermFEM {
	...
	double
	QuadrilateralLinearLocal2D::PsiDKsi( size_t const IntegrationPointIndex, size_t const Index ) {
		if( IntegrationPointIndex >= m_Ksi.size() ) {
			throw std::runtime_error(
	...
	double
	QuadrilateralLinearLocal2D::PsiDEta( size_t const IntegrationPointIndex, size_t const Index ) {
		if( IntegrationPointIndex >= m_Ksi.size() ) {
			throw std::runtime_error(
