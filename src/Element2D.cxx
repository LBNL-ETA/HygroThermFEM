...
namespace MoisThermFEM {
	...
	ElementLinear2D::ElementLinear2D( const Node2D & t_Node1, const Node2D & t_Node2,
			...
			: m_Element( t_Node1, t_Node2, t_Node3, t_Node4 ),
			  m_ElementNodes( t_Node1, t_Node2, t_Node3, t_Node4 ), m_Cond( t_Cond ), m_Rho( t_Rho ),
			  m_Cp( t_Cp ), m_Conductivity( numOfElements, std::vector< double >( numOfElements, 0 ) ),
			  m_RhoCp( numOfElements, std::vector< double >( numOfElements, 0 ) ) {

		const auto count = IntegrationPoints2D::Instance().count2D();

		for( unsigned i = 0; i < count; ++i ) {
			auto ConductionInIntegrationPoint = calculateConductionMatrix( i );
			auto RhoCPInIntegrationPoint = calculateRhoCpMatrix( i );
			for( unsigned j = 0; j < numOfElements; ++j ) {
				for( unsigned k = 0; k < numOfElements; ++k ) {
					m_Conductivity[ j ][ k ] += ConductionInIntegrationPoint[ j ][ k ];
	...
	ElementLinear2D::ElementLinear2D( const ElementLinear2D & t_Element ) : m_Element(
			t_Element.m_Element ), m_ElementNodes( t_Element.m_ElementNodes ), m_Cond(
			t_Element.m_Cond ) {
		m_Conductivity.resize( t_Element.m_Conductivity.size() );
		for( size_t i = 0; i < m_Conductivity.size(); ++i ) {
			for( const auto aValue : t_Element.m_Conductivity[ i ] ) {
		...
		for( size_t i = 0; i < m_RhoCp.size(); ++i ) {
			for( const auto aValue : t_Element.m_RhoCp[ i ] ) {
				m_RhoCp[ i ].push_back( aValue );
			}
		}
	}

	std::vector< size_t > ElementLinear2D::nodeIndexes() const {
		return m_ElementNodes.getNodeIndexes();
	}

	std::vector< std::vector< double > > ElementLinear2D::conductivity() const {
		return m_Conductivity;
	}

	std::vector< std::vector< double > > ElementLinear2D::RhoCp() const {
		return m_RhoCp;
	}

	std::vector< std::vector< double > >
	ElementLinear2D::calculateConductionMatrix( const std::size_t t_IntegrationPointIndex ) const {
		std::vector< std::vector< double > > aMatrix( numOfElements,
				std::vector< double >( numOfElements, 0 ) );
		...
		for( size_t i = 0; i < numOfElements; ++i ) {
			for( size_t j = 0; j < numOfElements; ++j ) {
				aMatrix[ i ][ j ] =
						( DPsiDx[ i ] * DPsiDx[ j ] + DPsiDy[ i ] * DPsiDy[ j ] ) * det * m_Cond;
			}
		}
	...
	std::vector< std::vector< double > >
	ElementLinear2D::calculateRhoCpMatrix( const std::size_t t_IntegrationPointIndex ) const {
		std::vector< std::vector< double > > aMatrix( numOfElements,
				std::vector< double >( numOfElements, 0 ) );
