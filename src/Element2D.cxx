...
IQLEMatrix2D::IQLEMatrix2D(const QuadrilateralLinearGlobal2D &t_Element)
    : m_Global2D{t_Element}, m_IntegrationMatrix{
                                 numOfQuadrilateralNodes,
                                 SquareMatrix{numOfQuadrilateralNodes}} {}
...
IQLEMatrix2D::integrate(const std::vector<double> &t_Values) const {
  ...
  std::vector<std::vector<double>> aMatrix(
      numOfQuadrilateralNodes, std::vector<double>(numOfQuadrilateralNodes, 0));
...
QLEConductance2D::QLEConductance2D(const QuadrilateralLinearGlobal2D &t_Element)
  ...
  for (std::size_t integrationPoint = 0;
       integrationPoint < numOfIntegrationPoints; ++integrationPoint) {
...
IElementLinear2D::IElementLinear2D(const Node2D &t_Node1, const Node2D &t_Node2,
    ...
    : m_Material{t_Material}, m_Node{{t_Node1, t_Node2, t_Node3, t_Node4}},
      m_Global2D{t_Node1, t_Node2, t_Node3, t_Node4},
      m_QLECapacitance2D{m_Global2D}, m_QLEConductance2D{m_Global2D} {
...
IElementLinear2D::conductanceDerivativeMatrix() {
  FenestrationCommon::SquareMatrix result{numOfQuadrilateralNodes};
  const auto numOfIntegrationPoints = IntegrationPoints2D::Instance().count2D();
...
const Material &IElementLinear2D::getMaterial() const { return m_Material; }

bool IElementLinear2D::haveBothNodes(const Node2D &t_Node1,
                                     const Node2D &t_Node2) const {
...
ElementThermalLinear2D::ElementThermalLinear2D(const Node2D &t_Node1,
  ...
  auto waterCapacitance =
      waterFill * (Constants::Density_Water * Constants::Cp_Water);
  ...
  /// Seems that it is not neccessary to create separate term when conductance
  /// coefficient is changing. Need to confirm that with rest of the team
  /// (Simon)
...
ElementMoistureLinear2D::ElementMoistureLinear2D(const Node2D &t_Node1,
  ...
  //////////////////////////////////////////////////////////////////////////////
  iValue delta(std::make_shared<MoisThermFEM::Constant>(
      2.5E-5 / mat.diffusionResistanceFactor()));
  iValue saturationFunction(std::make_shared<MoisThermFEM::SaturationFunction>(
  ...
  //////////////////////////////////////////////////////////////////////////////
  iValue suctionCurve(std::make_shared<MoisThermFEM::SuctionFunction>(
      mat.liquidTransportationCurve(), Property::humidity));
  m_Conductance.push_back(suctionCurve);

  //////////////////////////////////////////////////////////////////////////////
  /// Creating capacitance function
  //////////////////////////////////////////////////////////////////////////////
  iValue sorptionDerivative(std::make_shared<MoisThermFEM::TabularDerivative>(
      mat.sorptionCurve(), Property::humidity));
