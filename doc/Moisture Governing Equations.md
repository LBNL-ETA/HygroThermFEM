...
| **D6-refine** capacity | **New: found by the D1 diagnostic** | The closed-strip T-gradient diagnostic (`VaporTemperatureDiffusion` + `examples/compare_d1.py`) shows D1 is correct (isothermal exact; gradient matches the reference's hot→cold direction) but the engine **leaks ~1% mass** where the reference conserves to 1e-12, and its profile diverges ~10%. Cause: the moisture capacity is assembled via the coefficient-averaged `∫ξψᵢψⱼ` matrix then lumped, which only *approximately* conserves. **Fix: assemble the moisture capacity as a pure nodal-lumped secant diagonal `vᵢ·ξᵢ`** (as the reference does) → exact conservation, and should close most of the engine↔reference gap and reduce the `ExtremeHumidity` violations. Moisture-only, thermal-safe. Highest-value next step. |
